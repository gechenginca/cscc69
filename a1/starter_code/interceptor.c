...
/* sys_call_table[] stores the system call numbers and the address of the kernel function to call
 */
extern void* sys_call_table[];

/* The sys_call_table is read-only => must make it RW before replacing a syscall */
/* Page Table Entries (PTE) of type pte_t, which finally points to page frames containing the actual user data
 */
 ...
 * A list is headed by a structure defined by the LIST_HEAD macro. This structure contains a single pointer to the first element on the list. LIST_HEAD(HEADNAME, TYPE) head;
 */
struct pid_list {
  pid_t pid;
...
/*
 * asmlinkage: gcc tells the compiler that the function should not expect to find any of its arguments in registers (a common optimization), but only on the CPU's stack.
 */
typedef struct {
...
void my_exit_group(int status){
  // Remove this pid from all lists
  del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
  // Log message for some monitored pids
  if (table[reg.ax].monitored == 1) {
    if (check_pid_monitored(reg.ax, current->pid)) {
      log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
    }
  }
  // Log message for all monitored pids
  else if (table[reg.ax].monitored == 2) {
    log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
  }
  // Call the original system call, pass reg as param of f
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
  ...
  if (cmd == REQUEST_SYSCALL_INTERCEPT) {
    ...
    if (table[syscall].intercepted == 1) { return -EBUSY; }
      ...
      // Intercept original system call
      table[syscall].intercepted = 1;
      // Hijack original system call
    ...
    // Validate root permission
    if (current_uid() != 0) { return -EPERM; }
    // Validate de-interception
    // Syscall has not been intercepted yet
    if (table[syscall].intercepted == 0) { return -EINVAL; }
    else {
      // De-intercept original system call
      table[syscall].intercepted = 0;
      // Destroy the list of pids of this syscall and update the status for this table[syscall]
      destroy_list(syscall);
      // Release original system call
      spin_lock(&calltable_lock);
      set_addr_rw((unsigned long)sys_call_table);
    ...
    // Validate root permission and pid ownership
    if (current_uid() != 0) {
      if ((pid == 0) || check_pid_from_list(pid, current->pid)) { return -EPERM; }
    }
    // Validate monitoring
    spin_lock(&pidlist_lock);
    // pid is being monitored
    if (check_pid_monitored(syscall, pid) == 1) { spin_unlock(&pidlist_lock); return -EBUSY; }
    else {
      // This syscall monitors pids if pid!=0
      // This syscall monitors pids that are not being monitored if pid=0
      if (pid == 0) { // start monitoring all pids, clear the blacklist
          table[syscall].monitored = 2;
          destroy_list(syscall);
          spin_unlock(&pidlist_lock);
        }
      // table[syscall].monitored is 0 or 1
      else if (table[syscall].monitored != 2) {
        if (add_pid_sysc(pid, syscall)) { spin_unlock(&pidlist_lock); return -ENOMEM; }
        table[syscall].monitored = 1;
      } else { // table[syscall].monitoerd is 2, this pid is now being monitored, removing from the blacklist
        if (del_pid_sysc(pid, syscall)) { spin_unlock(&pidlist_lock); return -EINVAL; }
      }
      spin_unlock(&pidlist_lock);
    }
  }
    ...
    // Syscall has not been intercepted yet
    if (table[syscall].intercepted == 0) { return -EINVAL; }
    // Validate stop monitoring
    spin_lock(&pidlist_lock);
    // pid is not being monitored
    if (check_pid_monitored(syscall, pid) == 0) { spin_unlock(&pidlist_lock); return -EINVAL; }
    else {
      // This syscall monitors pids if pid!=0
      // This syscall monitors this pid that are not being monitored if pid=0
      if (pid == 0) {
        destroy_list(syscall);
        spin_unlock(&pidlist_lock);
      }
      // table[syscall].monitored is 0 or 1
        ...
        if (del_pid_sysc(pid, syscall)) { spin_unlock(&pidlist_lock); return -EINVAL; }
      } else { // table[syscall].monitoerd is 2, this pid is not being monitored, adding to the blacklist
        if (add_pid_sysc(pid, syscall)) { spin_unlock(&pidlist_lock); return -ENOMEM; }
      }
      spin_unlock(&pidlist_lock);
    }
...
static int init_function(void) {
  ...
  // Lock calltable_lock. A spinlock is a lock which causes a thread trying to acquire it to simply wait in a loop ("spin") while repeatedly checking if the lock is available. "Busy Waiting".
  spin_lock(&calltable_lock);
  // Save original system call and exit group
  orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
  orig_exit_group = sys_call_table[__NR_exit_group];
  // Hijack original system call and exit group
  set_addr_rw((unsigned long)sys_call_table);
...
static void exit_function(void){
  ...
  // Restore original system call and exit group
  set_addr_rw((unsigned long)sys_call_table);
